...
class Command(BaseCommand):
    """
    Команда для массового удаления данных из таблиц, связанных с указанными моделями и таблицами.

    Эта команда последовательно выполняет удаление данных из таблиц, используя две команды:
    - `delete_by_model`: для удаления данных из таблицы, найденной по имени модели Django.
    - `delete_by_table`: для удаления данных из таблицы, найденной по её имени.

    Пример использования:
        python manage.py delete_data

    Список команд и таблиц, из которых удаляются данные, задается в переменной COMMANDS.
    Каждый элемент списка COMMANDS содержит:
    ...
    Где:
        - "delete_by_model" — команда для удаления данных из таблицы, связанной с моделью.
    ...
    Где:
        - "delete_by_table" — команда для удаления данных из таблицы, найденной по её имени.
        - "reviews_title_genre" — имя таблицы.

    Примечание:
        - Убедитесь, что модели и таблицы существуют в базе данных.
        - Команда удаляет все записи из указанных таблиц, но сами таблицы остаются.
    """
