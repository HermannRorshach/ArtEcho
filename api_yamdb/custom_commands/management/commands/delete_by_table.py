...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        """
        Основной метод, который выполняет команду. Получает имя таблицы и
        выполняет SQL-запрос для удаления всех записей из неё.
        Операция выполняется внутри транзакции для обеспечения атомарности.
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Подготавливаем данные к удалению из таблицы "{table_name}"'))
        with transaction.atomic():
            with connection.cursor() as cursor:
                cursor.execute(f'DELETE FROM {table_name}')
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Успешно удалили объекты модели '
                        f'{table_name} из соответствующей таблицы'))
