...
FILES = [
    ...
    ("import_by_model", "static/data/category.csv", "reviews.Category", "--bulk"),
...
class Command(BaseCommand):
    ...
    - `import_by_model`: для импорта данных в таблицу, найденную по модели Django.
    - `import_by_table`: для импорта данных в таблицу по её имени.

    Пример использования:
        python manage.py load_data

    Список файлов и моделей, в которые импортируются данные, задается в переменной FILES.
    ...
    - Опционально: флаг `-b` или `--bulk` для команды `import_by_model`, который указывает
      на использование массовой вставки данных с помощью `bulk_create`. Если флаг не указан,
      данные вставляются по одному объекту с вызовом метода `save()`.
    ...
    Примечание:
        ...
        - Команда автоматически определяет формат файла (CSV или JSON) по его расширению.
    """
    ...
    def handle(self, *args, **options):
        ...
        Для команды `import_by_model` можно использовать флаг `-b` или `--bulk`,
        чтобы включить массовую вставку данных с помощью `bulk_create`.
        """
        for file in FILES:
            call_command(*file)